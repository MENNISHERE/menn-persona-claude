...
export default function StructuredData() {
  useEffect(() => {
    // ID-linked Schema.org Graph to fully isolate "MENN" as an independent brand entity
    const schema = {
    ...
    // Find or create the JSON-LD script element
    let script = document.getElementById('json-ld-seo-schema') as HTMLScriptElement | null;
    if (!script) {
    ...
    return () => {
      // Clean up the dynamically injected script if component unmounts
      const scriptToRemove = document.getElementById('json-ld-seo-schema');
      if (scriptToRemove) {
